...
import { 
  PlayCircle, Trophy, Target, Award, Zap, 
  Activity, Star, Ban, ShieldAlert, Info, 
  Maximize2, X, 
  PlayIcon
} from 'lucide-react';
...
const VideoSection = () => {
  ...
  const videoUrl = "https://res.cloudinary.com/debuweamw/video/upload/v1766505923/WhatsApp_Video_2025-12-11_at_3.08.16_PM_i1f6yo.mp4"; 
      ...
      <div className="relative transform -skew-x-3 bg-slate-900 border-4 border-slate-800 shadow-2xl overflow-hidden group hover:border-red-600/50 transition-colors duration-500 aspect-video max-w-6xl mx-auto">
        ...
        {isPlaying ? (
          <div className="absolute inset-0 z-40 bg-black">
            ...
            {/* Optional: Close button to return to thumbnail */}
            <button 
                onClick={() => setIsPlaying(false)}
                className="absolute top-4 right-4 z-50 p-2 bg-red-600/80 hover:bg-red-600 text-white transform skew-x-3"
            >
                <X size={20} />
            </button>
          </div>
          ...
          <>
            <div className="absolute inset-0 bg-gradient-to-tr from-red-600/20 via-transparent to-black/40 opacity-60 z-10 pointer-events-none"></div>
            
            
            
            {/* Scan line animation */}
            <div className="absolute top-0 left-0 w-full h-1 bg-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.5)] animate-[scan_3s_linear_infinite] z-20 pointer-events-none"></div>
            <style>{`@keyframes scan { 0% { top: -10%; opacity: 0; } 10% { opacity: 1; } 90% { opacity: 1; } 100% { top: 110%; opacity: 0; } }`}</style>

            <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-6 text-center z-30 transform skew-x-3">
              <button 
                onClick={() => setIsPlaying(true)}
                className="group/btn relative"
              >
                <div className="w-24 h-24 bg-red-600 flex items-center justify-center transform -skew-x-12 border-2 border-red-400 shadow-[0_0_30px_rgba(220,38,38,0.4)] group-hover/btn:scale-110 transition-transform duration-300">
                   <PlayIcon size={48} className="text-white transform skew-x-12 fill-current" />
                </div>
              </button>
              <h3 className="mt-8 text-4xl md:text-6xl font-black italic uppercase tracking-tighter text-transparent text-stroke-white">
                WATCH THE <span className="text-white text-stroke-0">ACTION</span>
              </h3>
            </div>

            <div className="absolute bottom-6 left-6 z-30 transform skew-x-3 bg-black/80 px-4 py-2 border-l-4 border-red-600">
              <span className="text-red-500 font-bold text-xs uppercase tracking-widest">Duration // 2:51</span>
            </div>
          </>
        )}
      </div>
...
const EquipmentCard = ({ title, imageSrc, desc, items, icon: Icon, onImageClick }) => (
    ...
    <div className="relative z-10 p-6 flex flex-col h-full transform -skew-x-12">
        ...
        <div className="transform skew-x-12 h-full flex flex-col">
            ...
            >
                ...
                <img src={imageSrc} alt={title} className="w-full h-full object-cover opacity-80 group-hover/img:opacity-100 group-hover/img:scale-110 transition-all duration-500" />
                ...
                <div className="absolute bottom-4 left-4 z-20">
                    <h3 className="text-2xl font-black italic text-white uppercase">{title}</h3>
                </div>
                
                <div className="absolute top-4 right-4 z-20 opacity-0 group-hover/img:opacity-100 transition-opacity">
                    <div className="bg-black/80 p-1.5 border border-slate-600"><Maximize2 size={16} className="text-white" /></div>
                </div>
            </div>

            <p className="text-slate-400 text-sm font-medium mb-6 flex-grow leading-relaxed border-l-2 border-slate-800 pl-4">{desc}</p>
            ...
            <div className="bg-black/40 p-4 border border-slate-800/50">
                ...
                <div className="space-y-2">
                    {items.map((item, i) => (
                        <div key={i} className="flex justify-between text-xs border-b border-slate-800 pb-1 last:border-0 last:pb-0">
...
const RulesList = () => (
    <div className="space-y-6">
        {[
            { id: "01", title: "The Setup", desc: "Competition begins in a 15x15m arena. Striker defends the X against 5 fielders.", icon: Target },
            { id: "02", title: "The Throw", desc: "Underarm delivery targeting upper X zone. Must range from hip to knee height.", icon: Zap },
            { id: "03", title: "Hit & Run", desc: "Striker launches the Sort into play. Fielders mobilize. Every completed base run adds to the tally.", icon: Activity }
        ].map((step, i) => (
            <div key={i} className="relative pl-8 group">
                ...
                <div className="bg-slate-900/50 border border-slate-800 p-6 relative overflow-hidden group-hover:border-slate-600 transition-all">
                    ...
                    <div className="absolute top-0 right-0 text-6xl font-black italic text-white opacity-5 group-hover:opacity-10 transition-opacity -mt-2 -mr-4 select-none">{step.id}</div>
                    <div className="relative z-10 flex gap-4 items-start">
                        <div className="text-red-500 mt-1"><step.icon size={24} /></div>
                        <div>
                            <h4 className="text-xl font-black italic text-white uppercase mb-2">{step.title}</h4>
    ...
    <div className="relative bg-black border-2 border-slate-800 p-8 transform -skew-x-3 shadow-2xl">
        <div className="transform skew-x-3">
             <h3 className="text-2xl font-black italic text-white mb-8 uppercase border-b border-slate-800 pb-4 flex items-center gap-3">
                <div className="bg-red-600 p-1.5"><Activity size={20} className="text-white" /></div>
                Scoring Intel
             </h3>
             
             <div className="space-y-6">
                <div className="flex gap-4 p-4 bg-green-900/10 border border-green-900/30">
                    <div className="text-green-500"><Star size={24} /></div>
                    <div>
                        <strong className="block text-green-400 font-bold uppercase text-sm mb-1">Point Earned</strong>
                        <p className="text-xs text-slate-400 leading-relaxed">Successful in-bounds hit or completed run between bases.</p>
                    </div>
                </div>

                <div className="space-y-3">
                    {[
                        { icon: Ban, color: "text-red-500", title: "Direct Hit", desc: "Sort contacts X. Immediate Elimination." },
                        { icon: ShieldAlert, color: "text-orange-500", title: "Boundary Breach", desc: "Hit lands outside arena bounds." },
                        { icon: Trophy, color: "text-rose-500", title: "Aerial Catch", desc: "Fielder secures Sort mid-flight." }
                    ].map((rule, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 bg-slate-900/50 border-l-2 border-slate-700 hover:border-red-500 transition-colors">
                            <rule.icon size={18} className={rule.color} />
                            <div>
                                <div className="text-white font-bold text-xs uppercase">{rule.title}</div>
                                <div className="text-slate-500 text-[10px] uppercase font-bold tracking-wider">{rule.desc}</div>
                            </div>
                        </div>
                    ))}
                </div>
...
export default function SectionRules() {
  ...
  return (
    <section id="rules" className="relative min-h-screen bg-slate-950 font-sans overflow-hidden pt-32 pb-24 text-slate-100">
      {/* Font Injection */}
      <style>{`
      ...
      <div className="container mx-auto px-6 relative z-10">
        <RulesHeader 
            title={<>MASTER THE <span className="text-red-600">ARENA</span></>}
        ...
        <div className="mb-32">
            <div className="flex items-center gap-4 mb-12">
                ...
                <h3 className="text-2xl font-black italic text-slate-500 uppercase tracking-widest">Equipments</h3>
                <div className="h-px bg-slate-800 flex-grow"></div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 md:gap-6 px-4">
                <EquipmentCard 
                  ...
                  items={[{label: "Length", value: "25-28\""}, {label: "Zone", value: "14-15\""}, {label: "Weight", value: "1.25 kg"}]}
                />
                <EquipmentCard 
                  title="The Sort"
                  imageSrc={sort}
                  ...
                  icon={Target}
                  onImageClick={setSelectedImage}
                  items={[{label: "Shape", value: "Cubic"}, {label: "Mat", value: "Leather"}, {label: "Weight", value: "200g"}]}
                />
                <EquipmentCard 
                  title="The Target"
                  imageSrc={target}
                  ...
                  icon={Award}
                  onImageClick={setSelectedImage}
                  items={[{label: "Height", value: "30\""}, {label: "Width", value: "2\""}, {label: "Base", value: "Weighted"}]}
                />
            </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
            <div className="lg:col-span-7">
                <h3 className="text-3xl font-black italic text-white mb-8 uppercase border-l-4 border-red-600 pl-4">Rules of Engagement</h3>
                <RulesList />
            </div>
      ...
      <AnimatePresence>
        {selectedImage && (
          ...
          >
            <div className="absolute top-8 right-8 cursor-pointer" onClick={() => setSelectedImage(null)}>
                <X size={32} className="text-white hover:text-red-500 transition-colors" />
            </div>
            <motion.img 
            ...
            />
             <div className="absolute bottom-10 left-0 w-full text-center pointer-events-none">
                 <span className="bg-black/80 text-white px-4 py-2 text-xs font-bold uppercase tracking-widest border border-slate-800">Press ESC to Close</span>
             </div>
          </motion.div>
        )}
      </AnimatePresence>
